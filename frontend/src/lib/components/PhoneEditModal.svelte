    ...
    >
      <div class="mb-4">
        ...
        <p class="text-sm text-slate-600 mb-4">
          {$t('reminder.retry.edit_phone_message', { values: { name: patientName } })}
        </p>

        <div class="space-y-2">
          <label for="phone-input" class="block text-sm font-medium text-slate-700">
            {$t('patient.phone')}
          </label>
          <input
            id="phone-input"
      ...
      <div class="flex flex-col sm:flex-row gap-3">
        ...
        <button
          ...
          disabled={!isValid}
          class="flex-1 px-4 py-2.5 bg-emerald-600 text-white font-medium rounded-xl hover:bg-emerald-700 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
        >
