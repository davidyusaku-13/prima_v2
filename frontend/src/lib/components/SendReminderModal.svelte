    ...
    >
      ...
      {#if patient}
        <div class="mb-4 p-3 bg-slate-50 rounded-xl">
          <p class="text-xs text-slate-500 mb-1">{$t('patients.patient') || 'Patient'}</p>
      ...
      <p class="text-sm text-slate-700 mb-4">
        {$t('reminder.send.confirm', { values: { patientName: patient?.name || '' } })}
      </p>
      ...
      <div class="flex flex-col sm:flex-row gap-3">
        <button
          ...
          class="flex-1 px-4 py-2.5 border border-slate-200 text-slate-700 font-medium rounded-xl hover:bg-slate-50 transition-colors duration-200 order-2 sm:order-1 disabled:opacity-50 disabled:cursor-not-allowed"
        >
        ...
        <button
          onclick={onConfirm}
          disabled={status === 'sending'}
          class="flex-1 px-4 py-2.5 bg-green-600 text-white font-medium rounded-xl hover:bg-green-700 transition-colors duration-200 order-1 sm:order-2 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
        >
          {#if status === 'sending'}
