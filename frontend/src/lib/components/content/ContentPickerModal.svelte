<script>
  ...
  let totalResults = $derived.by(
    () => displayedArticles.length + displayedVideos.length
  );

  // Check if content is empty (no articles and no videos)
  let hasNoContent = $derived.by(
    () => articles.length === 0 && videos.length === 0
  );

  // Check if search has no results
  let hasNoSearchResults = $derived.by(
    () => !hasNoContent && totalResults === 0 && debouncedQuery.trim() !== ""
  );

  // Check if initial state (no search, has content)
  let hasInitialState = $derived.by(
    () => !hasNoContent && !hasNoSearchResults && debouncedQuery.trim() === ""
  );
    ...
    return recentAttachments
      .filter(
        (item) =>
          ...
          (articles.some((a) => a.id === item.id) ||
            videos.some((v) => v.id === item.id))
      )
      .slice(0, MAX_RECENT);
  });
  ...
  function isSelected(content) {
    return selectedContent.some(
      (c) => c.id === content.id && c.type === getContentType(content)
    );
  }
  ...
  function saveRecentAttachments() {
    try {
      localStorage.setItem(
        recentAttachmentsKey,
        JSON.stringify(recentAttachments)
      );
    } catch (e) {
  ...
  function addToRecentAttachments(content) {
    ...
    recentAttachments = recentAttachments.filter(
      (a) => a.id !== content.id || a.type !== type
    );
  ...
  function handleSelect(content) {
    ...
    if (isSelected(content)) {
      // Deselect
      onSelect(
        selectedContent.filter((c) => !(c.id === content.id && c.type === type))
      );
    } else {
  ...
  // Ensure no pending debounced search timers leak across unmounts
  onDestroy(() => {
    if (searchTimeout) {
      clearTimeout(searchTimeout);
      searchTimeout = null;
...
{#if show}
  ...
  >
    ...
    >
      ...
      <div class="flex-shrink-0 p-4 sm:p-6 border-b border-slate-100">
        ...
        <div class="relative">
          ...
          <input
            ...
            type="text"
            placeholder={$t("content.picker.search") ||
        ...
        <div class="flex gap-2 mt-4" role="tablist" aria-label="Jenis konten">
          {#each [{ id: "all", label: $t("content.picker.tab.all") || "Semua" }, { id: "article", label: $t("content.picker.tab.articles") || "Artikel" }, { id: "video", label: $t("content.picker.tab.videos") || "Video" }] as tab}
            <button
        ...
        {#if selectedContent.length > 0}
          <div
            class="mt-3 px-3 py-2 bg-teal-50 rounded-lg flex items-center justify-between"
          >
      ...
      >
        ...
        {:else if hasNoContent}
          ...
          <EmptyState
            ...
            actionLabel={isAdmin
              ? $t("cms.addArticle") || "Tambah Konten"
              : undefined}
            onAction={isAdmin
        ...
        {:else if hasNoSearchResults}
          ...
          <EmptyState
            ...
            title={$t("content.picker.noResults").replace(
              "{query}",
              `"${searchQuery}"`
            ) || `Tidak ada hasil untuk "${searchQuery}"`}
            description={$t("content.picker.noResultsGuidance") ||
              "Coba kata kunci lain atau browse kategori"}
          />
          <!-- Popular Content suggestions -->
          {#if suggestedContent.length > 0}
            <div class="mt-4">
              <h4
                class="text-sm font-medium text-slate-500 mb-3 uppercase tracking-wide"
              >
                ...
                {#each suggestedContent as item (item.id + "-" + item.type)}
                  ...
                  >
                    <div class="flex items-center gap-3">
                      <div
                        class="flex-shrink-0 w-12 h-12 bg-slate-100 rounded-lg overflow-hidden"
                      >
                        {#if item.thumbnail}
                          <img
                      ...
                      <div class="flex-1 min-w-0">
                        ...
                        <p class="text-xs text-slate-500 capitalize">
                          {item.type === "video" ? "Video" : "Artikel"} · {item.source ===
                          "recent"
                            ? $t("content.picker.recentLabel") || "Baru saja"
                            : $t("content.picker.popularLabel") || "Populer"}
                        </p>
                      </div>
                    </div>
                  </button>
                    ...
                    <div class="flex items-start gap-3">
                      ...
                      <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2 mb-1">
                          <span
                            class="inline-block px-2 py-0.5 bg-teal-50 text-teal-700 text-xs font-medium rounded"
                          >
                            {$t("content.picker.articles") || "Artikel"}
                          </span>
                      ...
                      <div class="flex-shrink-0">
                        ...
                        {:else}
                          <div
                            class="w-6 h-6 rounded-full border-2 border-slate-300"
                          ></div>
                        {/if}
                      </div>
                    </div>
                    ...
                    <div class="flex items-start gap-3">
                      ...
                      >
                        {#if video.thumbnail_url}
                          ...
                          {#if video.duration}
                            <div
                              class="absolute bottom-1 right-1 px-1 bg-black/70 text-white text-xs rounded"
                            >
                              {video.duration}
                            </div>
                          {/if}
                      ...
                      <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2 mb-1">
                          <span
                            class="inline-block px-2 py-0.5 bg-purple-50 text-purple-700 text-xs font-medium rounded"
                          >
                            {$t("content.picker.videos") || "Video"}
                          </span>
                        </div>
      ...
      <!-- Footer -->
      <div
        class="flex-shrink-0 p-4 sm:p-6 border-t border-slate-100 bg-slate-50"
      >
        <div class="flex flex-col sm:flex-row gap-3">
          ...
          >
            <span
              >{$t("content.picker.attach") || "Lampirkan"} ({selectedContent.length})</span
            >
          </button>
        </div>
      </div>
