      ...
      <div class="flex items-center gap-3 sm:gap-4 mb-4 sm:mb-6">
        ...
        <div>
          <p class="text-slate-700 font-medium">
            {$t("reminder.cancel.title") || "Batalkan Reminder?"}
          </p>
          <p class="text-slate-500 text-sm mt-1">
            {$t("reminder.cancel.confirm") ||
              "Apakah Anda yakin ingin membatalkan reminder ini?"}
          </p>
        </div>
      </div>
      ...
      <div class="bg-slate-50 rounded-lg p-3 mb-4">
        ...
        {#if reminder.scheduled_at || reminder.due_date}
          <div class="flex items-center gap-1.5 mt-2 text-xs text-slate-600">
            ...
            <span
              >{$t("reminder.cancel.scheduledTime") || "Dijadwalkan"}: {new Date(
                reminder.scheduled_at || reminder.due_date
              ).toLocaleString($locale || "id", {
                dateStyle: "medium",
                timeStyle: "short",
              })}</span
            >
